...
func createTenantDatabase(ctx context.Context, adminDSN, dbName string) (string, error) {
	// dbName is generated by slugify() (lowercase alnum + '-' only, then '-'
	// replaced with '_'), but re-validate defensively since it's interpolated
	// into a CREATE DATABASE statement — Postgres has no parameterized DDL.
