...
// that instance and lands on its own dashboard). Light mode by default,
...
export default function CloudWaitPage() {
  ...
  useEffect(() => {
    ...
    const dark = localStorage.getItem('rr-theme') === 'dark'
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-[var(--cream)] px-4 font-sans">
      <div className="max-w-sm text-center bg-paper border border-[var(--border)] shadow-[5px_5px_0_rgba(92,58,30,.15)] px-8 py-10">
        <div className="flex flex-col items-center mb-6">
          <LogoMark size={32} color="currentColor" />
        </div>
        {!error && (
          <div className="w-9 h-9 mx-auto mb-5 rounded-full border-[3px] border-[var(--border)] border-t-[var(--gold)] animate-spin" />
        )}
        <h2 className="text-lg font-bold text-[var(--text)] mb-2">
          {error ? 'Setup failed.' : 'Setting up your workspace\u2026'}
        </h2>
        {!error && <p className="text-sm text-[var(--text-mid)]">This usually takes under a minute.</p>}
